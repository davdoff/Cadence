...
- `EventActivityAttributes` (`Models/Shared/`) is the shared attributes type — member of **both** targets. `LiveActivityService` (app target) calls `Activity.request(…)` on start and `activity.end(…, .immediate)` on completion/skip. `EventLiveActivity` (widget target, added to `CadenceWidgetBundle`) renders the Lock Screen banner + island regions; the expanded island shows a `ProgressView(timerInterval:)` bar tinted with the event's category/accent color. Requires `NSSupportsLiveActivities = YES` in the app `Info.plist`.
- **Stop button** — the Lock Screen view and expanded island host an interactive `Button(intent: StopEventIntent(...))`. `StopEventIntent` (`Models/Shared/`, both targets) is a `LiveActivityIntent`, so `perform()` runs in the **app** process. It deliberately uses only cross-target APIs (SwiftData models, ActivityKit, WidgetKit, UserNotifications) — completing the event, ending the activity, and reloading widgets inline — so it compiles identically in both targets with **no compilation flag**. Its completion logic mirrors `EventActionService.complete`; keep the two in sync.
