...
- **Start-event timer & auto-complete** — in the Today tab, tapping a startable event springs the card narrower and reveals a **Start** pill (`StartableEventRow` in `TodayView`); tapping the card again (or off it) collapses it. Startability is gated by `Event.canStart` (`status != .completed && !isRunning`): a **completed** event shows no Start pill (restarting wouldn't track), and starting a **missed** event revives it to `.pending` (in `EventActionService.start`) so its timer actually shows and it tracks. The trailing pill/timer lives in an animatable fixed-width slot, so the card shrinks *as* the pill widens (one layout change, no pop-in), and the pill matches the card height (radius 18). Today intentionally does *not* open event detail (that lives on the Schedule tab, which opens `EventDetailView` on tap). Tapping Start stamps `Event.startedAt`, swaps the pill for a live `Text(timerInterval:)` countdown (self-updating, no timers), and schedules a "done!" local notification for `startedAt + duration` (`scheduleEventCompletionAlert`, identifier `event-finished-<id>`). **Tapping a running event's timer reveals a Stop pill** in its place; Stop *cancels* the timer (`EventActionService.cancelTimer` — clears `startedAt`, drops the completion alert, ends the Live Activity, reverts to `.pending`) as an undo for an accidental Start. Completing is done by swiping, not by Stop. When the countdown ends the event is **auto-marked complete** — via the same `mark(.completed)` path (so correlated habits still increment). Completion is triggered live by a per-row `.task` while the app is foreground, and by a scenePhase/`onAppear` reconciliation pass (`completeElapsedRunningEvents`) for events that finished while the app was closed; both are guarded against double-counting. Start / complete / miss all funnel through `EventActionService` so Today, the notification actions, and the Live Activity Stop button stay consistent. Swiping right on an **already-completed** event undoes it (`EventActionService.revertToPending` — decrements the correlated habit, clears started/completed state, back to `.pending`). Today's **"All"** filter shows the working set only (`status != .completed`); completed events appear solely under the **"Done"** filter.
...
- `EventActivityAttributes` (`Models/Shared/`) is the shared attributes type — member of **both** targets. `LiveActivityService` (app target) calls `Activity.request(…)` on start and `activity.end(…, .immediate)` on completion/skip; `start(for:)` bails if an activity for that `eventID` already exists, so it can't stack duplicates. `EventLiveActivity` (widget target, added to `CadenceWidgetBundle`) renders the Lock Screen banner + island regions; both the **Lock Screen** view and the expanded island show a `ProgressView(timerInterval:)` bar tinted with the event's category/accent color (tint is the only styling the timer-based bar supports). Requires `NSSupportsLiveActivities = YES` in the app `Info.plist`.
- **"Done" button** — the Lock Screen view and expanded island host an interactive `Button(intent: StopEventIntent(...))` labelled **Done** (checkmark); it *completes* the event (unlike the Today "Stop" pill, which only cancels the timer). `StopEventIntent` (`Models/Shared/`, both targets) is a `LiveActivityIntent`, so `perform()` runs in the **app** process. It deliberately uses only cross-target APIs (SwiftData models, ActivityKit, WidgetKit, UserNotifications) — completing the event, ending the activity, and reloading widgets inline — so it compiles identically in both targets with **no compilation flag**. Its completion logic mirrors `EventActionService.complete`; keep the two in sync.
- **`EventActionService`** (app target) is the single source of truth for start / complete / miss — the OS-blind status + habit-streak logic plus its device-side side effects (notifications, Live Activity, widget refresh). `TodayView` (`start`/`mark`), `NotificationDelegate` (start/skip), and `StopEventIntent` all funnel through it, so every surface stays consistent. It, the delegate, and the intent all use the one `SharedModelContainer.shared` container instance so writes from a notification/island action show up live in the UI's `@Query`.
- **Foreground/manual start only** — the activity begins from app code when the user starts an event; there is no auto-appear at the scheduled clock time (that would require ActivityKit **push-to-start** via APNs, out of scope — see `whiteboard.md`). An activity for an event completed while the app was closed lingers (greyed past its stale date) until the next launch runs the auto-complete reconciliation, which ends it.
